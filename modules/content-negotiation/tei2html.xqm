...
declare function tei2html:summary-view($nodes as node()*, $lang as xs:string?, $id as xs:string?) as item()* {
  ...
  return 
      ...
      else if(contains($id,'/bibl/') or contains($id,'/cbss/')) then tei2html:summary-view-bibl($nodes, $id)
...
declare function tei2html:summary-view-bibl($nodes as node()*, $id as xs:string?) as item()* {
    ...
    return 
        <div class="short-rec-view">
            ...
            {
            if($nodes/descendant-or-self::tei:relation[@ref='dc:subject']) then 
                <div class="results-list-desc">
                    {
                        for $subject in $nodes/descendant-or-self::tei:relation[@ref='dc:subject']
                        let $linked := (<a href="{$config:nav-base}/cbss/search.html?facet-cbssKeywords={$subject}">{normalize-space($subject)}</a>)
                        order by $subject
                        return 
                            ($linked,if($subject[position()=last()]) then '; ' else())
                    }
                </div>
            else()
            }
